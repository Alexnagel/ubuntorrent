...
class Serie
{
    ...
    public function __construct($data)
    {
        ...
        $this->actors = (string)implode(", ", Client::removeEmptyIndexes(explode('|', (string)$data->Actors)));
        ...
        $this->genres = (string)implode(", ", Client::removeEmptyIndexes(explode('|', (string)$data->Genre)));
    ...
    {
        $base_url = "http://www.thetvdb.com/banners/";
        if(file_exists(public_path() . '/' . $this->poster))
        {
            return true;
        }
        ...
        curl_setopt($ch, CURLOPT_URL, $base_url . $this->poster);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
        $tmp = curl_exec($ch);
        curl_close($ch);

        file_put_contents(public_path() . '/' . $this->poster , $tmp);
        return true;
    }
}